...
//   };
  ...
  let login = async (event) => {
    ...
    if (result.success) {
      ...
      let parentResult = await parent.json();
      console.log(parentResult)
      if (parentResult.success) {
        localStorage.setItem("parent", JSON.stringify(parentResult.parent));
        navigate("/parent-dashboard");
      } else {
        toast.error(
          parentResult.errors[0].msg, {
          position: "top-right",
          autoClose: 3000,
    ...
    <div className="w-full rounded-lg md:mt-0 sm:max-w-md xl:p-0 bg-gray-800 border-gray-700">
      <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
        <h1 className="text-xl font-bold leading-tight tracking-tight md:text-2xl text-white">
          Sign in to your account - Parents
        </h1>
