...
//     emailjs.send('service_wdkz8ro', 'template_tz32eam', {
//       email,
//       subject,
//       message,
//       // to_name,
...
function Contact() {
  ...
  const [subject, setSubject] = useState('');
  const [message, setMessage] = useState('');
  const [toName, setToName] = useState('Recipient Name'); // Placeholder for `to_name`
  const [fromName, setFromName] = useState('Your Name'); // Placeholder for `from_name`
  const [isSending, setIsSending] = useState(false);
  const [isSent, setIsSent] = useState(false);
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send('service_ft2zryt', 'template_tz32eam', {
      email,             // Corresponds to {{email}}
      subject,           // Corresponds to {{subject}}
      message,           // Corresponds to {{message}}
      to_name: toName,   // Corresponds to {{to_name}}
      from_name: fromName // Corresponds to {{from_name}} 
    }, 'GZukG0nAgSY69T0CM')
      .then(() => { 
        setIsSending(false);
        setIsSent(true);
