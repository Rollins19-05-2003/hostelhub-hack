...
function LeaveOff() {
  const getRequests = async () => {
    ...
    if (data.success) {
      data.list.map((req) => {
        req.id = req._id;
        req.from = new Date(req.start_date).toDateString().slice(4, 10);
        req.to = new Date(req.end_date).toDateString().slice(4, 10);
  ...
  const updateRequest = async (id, status) => {
    ...
    if (data.success) {
      ...
      toast.success(
        `Leave request from ${student.name} has been ${status}`,
        {
          position: "top-right",
          autoClose: 3000,
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center justify-center">
      ...
      <h1 className="text-white font-bold text-5xl">Manage Leave Requests</h1>
      ...
      <div className="bg-neutral-950 px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-96 overflow-auto">
        <span className="text-white font-bold text-xl">All Leave Requests</span>
